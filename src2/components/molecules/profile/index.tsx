...
const ProfileSummary: React.FC<any> = ({ profile, style }) => {
  ...
  const profileLogo = URL.createObjectURL(
    new Blob([logo], { type: logo.type }),
  );

  return (
    <section>
      <div
        className={`w-[70vw] p-4 bg-white relative rounded-lg h-auto text-left ${style}`}
      >
        <section>
          {profileLogo && (
            <img
          ...
          <section className="grid grid-cols-2 w-[70%]">
            <aside>
              <ProfileSummaryCard
                title="Diagnostic Center Name"
                value={profile?.diagnosticName}
              />
            ...
            <aside>
              <ProfileSummaryCard
                title="Branch Name"
                value={profile?.branchDetails?.[0]?.branchName}
              />
              <ProfileSummaryCard
                title="Branch Email"
                value={profile?.branchDetails?.[0]?.branchEmail}
                link={profile?.branchDetails?.[0]?.branchEmail}
              />
              <ProfileSummaryCard
                title="Branch Contact"
                value={profile?.branchDetails?.[0]?.branchContact}
              />
              <ProfileSummaryCard
                title="Branch Address"
                value={profile?.branchDetails?.[0]?.branchAddress}
              />
            </aside>
          </section>
        </section>
          ...
          <section>
            {profile?.brandingInfo?.logoUrl && (
              <img
                ...
                alt="logo"
                className="w-[18vw] h-[18vw] sm:w-[7vw] sm:h-[7vw] rounded-full border-2"
              />
            )}
