...
const PreviewComponent = ({ showPreview, onClose, request }) => {
      ...
      <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50 w-full">
        <div className="bg-white rounded-lg p-6 w-full md:w-3/4 lg:w-100 max-h-[80vh] overflow-auto" >
          ...
          <div className="preview-content" ref={componentRef} >
            <DiagnosticReport report={request} />
          </div>
          <div className="mt-6 flex justify-end">
            <button onClick={exportPDF} disabled={pdfLoading} className="bg-blue-500 hover:bg-blue-600 text-white py-2 px-4 rounded-md mr-2 focus:outline-none">
              {pdfLoading ? 'Exporting...' : 'Export PDF'}
            </button>
            <button onClick={onClose} className="bg-gray-200 text-gray-700 py-2 px-4 rounded-md hover:bg-gray-300 focus:outline-none">
              Close
            </button>
          </div>
        </div>
