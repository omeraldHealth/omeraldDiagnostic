import { useEditTestIdValues, useEditTestValues, useTestDetailValue } from "@components/common/constants/recoilValues";
import React, { useEffect } from "react";
import { ViewParameters } from "./parameters/view";
import { Button } from "antd";
import { useCreateTest, useGetDcBranch, useInvalidateQuery, useUpdateBranch, useUpdateTest } from "@utils/reactQuery";
...
export default function TestSummary({handlePrevious,handleShowTest}) {
    ...
    const { data: currentBranch, refetch, isLoading } = useGetDcBranch({ selectedBranchId: selectedBranch })
    ...
    const handleSubmit = () => { 
        ...
        if (!isLoading && !editTest) { 
            createTest.mutate({ data: testDetails }, {
                onSuccess: (res) => {
                    if (res?.status === 201) {
                        const updatedTest = [...(currentBranch?.data?.tests || []), res?.data?._id];
                        updateBranch.mutate(
                ...
                onError: (res) => {  errorAlert("Adding test failed")}
            })
        }

        if (editTest) { 
            updateTest?.mutate({ data: testDetails, recordId: editTestId }, {
                onSuccess: (resp) => {
                    if (resp.status == 200) { 
    ...
    return (
        <div className="p-10 h-auto bg-signBanner">
            ...
            <section className="my-5 text-gray-600">
                <p className="mb-10">Name: <span className="font-bold">{testDetails?.testName}</span>   </p>
                <p className="mb-10">Sample: <span className="font-bold">{testDetails?.sampleName}</span>   </p>
            </section>
            <ViewParameters />
