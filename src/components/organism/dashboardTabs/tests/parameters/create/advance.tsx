    ...
    return (
      <>
        <Form.Item label="Select Range Option" name="rangeOption">
            <Select placeholder="Select Range Type" onChange={handleRangeOptionChange} value={rangeOption}>
            <Option value="age">Age Range</Option>
            <Option value="gender">Gender Range</Option>
            </Select>
        </Form.Item>
        {rangeOption === 'age' ? <AgeRangeForm form={form} /> : <GenderRangeForm form={form} />}
      </>
    );
};
        ...
        {(fields, { add, remove }) => (
          <>
            {fields.map(({ key, name, fieldKey, ...restField }) => (
              <section key={key} style={{ display: '', alignItems: 'center', marginBottom: '8px' }}>
                <Form.Item
                  {...restField}
                  name={[name, 'gender']}
                ...
                {selectedGender && (
                  <Form.Item
                    ...
                    style={{ flex: 1, marginLeft: '8px' }}
                  >
                    <InputNumber placeholder="Min Value" style={{ width: '100%' }} />
                  </Form.Item>
                )}
                {selectedGender && (
                  <Form.Item
                    ...
                    style={{ flex: 1, marginLeft: '8px' }}
                  >
                    <InputNumber placeholder="Max Value" style={{ width: '100%' }} />
                  </Form.Item>
                        )}
                        {selectedGender && <FaTrash className='mt-2'
                            onClick={() => remove(name)}
            ...
            <Form.Item>
                <Button type="dashed" className='flex gap-5' onClick={() => add()} block icon={<FaPlus className='mt-1' />}>
                Add Gender Range
              </Button>
            </Form.Item>
          </>
...
const AgeRangeForm = ({ form }) => {
  ...
  return (
    <>
      ...
      >
        <Select placeholder="Select Age Range Type" onChange={handleAgeRangeTypeChange} value={ageRangeType}>
      ...
      {ageRangeType && (
        <Form.List name="ageRanges">
          {(fields, { add, remove }) => (
            <>
              {fields.map(({ key, name, fieldKey, ...restField }) => (
                <section key={key} style={{ display: 'flex', alignItems: 'center', marginBottom: '8px' }}>
                  <Form.Item
                    {...restField}
                    name={[name, 'unit']}
                    fieldKey={[fieldKey, 'unit']}
