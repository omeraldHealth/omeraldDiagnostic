...
const RenderRanges = ({ data, onRemove }) => {
    ...
    return (
        ...
        <List
            ...
            <List.Item
                key={index}
                actions={[<FaTrash onClick={() => onRemove('basicRange', index)} style={{ cursor: 'pointer' }} />]}
                >
                    <section className='flex '>
                        <p  className='mr-2'><strong>Unit:</strong> {item.unit}</p>
                        <p  className='mr-2'><strong>Min:</strong> {item.min}</p>
                        <p  className='mr-2'><strong>Max:</strong> {item.max}</p>
                    </section>
        ...
        {data?.advanceRange?.ageRange?.length > 0 && (
            ...
            <List
                ...
                <List.Item
                    key={index}
                    actions={[<FaTrash onClick={() => onRemove('ageRange', index)} style={{ cursor: 'pointer' }} />]}
                >
                    <section className='flex '>
                        <p  className='mr-2'><strong>Unit:</strong> {item.unit}</p>
                        <p  className='mr-2'><strong>Min:</strong> {item.min}</p>
        ...
        {data.advanceRange.genderRange.length > 0 && (
            ...
            <List
                ...
                <List.Item
                    key={index}
                    actions={[<FaTrash onClick={() => onRemove('genderRange', index)} style={{ cursor: 'pointer' }} />]}
                >
                     <section className='flex '>
                        <p  className='mr-2'><strong>Unit:</strong> {item.unit}</p>
                        <p  className='mr-2'><strong>Min:</strong> {item.min}</p>
        ...
        {data.advanceRange.customCategory.length > 0 && (
            ...
            <List
                ...
                <List.Item
                    key={index}
                    actions={[<FaTrash onClick={() => onRemove('customCategory', index)} style={{ cursor: 'pointer' }} />]}
                >
                    <Card>
                    {/* Display fields from customCategory schema */}
                    <Text><strong>Category Name:</strong> {item.categoryName}</Text><br />
