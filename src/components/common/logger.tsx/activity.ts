...
export function useActivityLogger() {
    ...
    const { data: profileData, refetch } = useGetDcProfile({ selectedCenterId: selectedDc });
    ...
    const logActivity = ({ activity }) => {
        try {
            ...
            const updatedBranch = { ...currentBranch, activities: [...(currentBranch?.activities || []), newActivity] };
